...
baltimore$`Lower confidence limit`
```

We can see that the estimated proportion of of unsolved homicides in Baltimore is `r baltimore$"Estimated proportion of unsolved homicides (%)"`%. We are 95% confident that the proportion of unsolved homicides in Baltimore is between `r baltimore$"Lower confidence limit"`% and `r baltimore$"Upper confidence limit"`%.

### Estimating the proportion of unsolved homicides across cities

While the code chunk above lets us compute the output for single city, we may want a more efficient way to get estimates for all cities. We will therefore use `map2` to apply the `prop.test` function to estimate the proportion of unsolved homicides in all cities in our dataset.
...
Based on the plot, we can see that for samples where the null hypothesis was rejected, average $\hat{\mu}$ estimates across tests for which the null is rejected is much lower than true value of $\mu$. This is in accordance to what we would expect, as samples with an estimated mean closer to 0 will not reject the null, while samples which were closer to the true mean value will reject the the null hypothesis.

Moreover, as the effect size $\mu$ increased, the estimated means between samples that rejected/did not reject the null deviated further apart from each other. Notably, for $\mu = 6$, since all samples led to a rejection of the null hypothesis, there is no $\hat{\mu}$ value, indicating that the effect size was too large to sample a distribution which falsely failed to reject the null. 
